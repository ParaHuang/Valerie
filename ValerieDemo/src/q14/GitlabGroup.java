...
//        Set<User> membersSet = members.keySet();	//get all keys->users
//        membersSet.stream().filter(user->members.get(user).equals(level))
//        					.forEach(user->names.add(user.getName()));
        
        members.entrySet().stream().filter(entry->entry.getValue().equals(level))	
        					.forEach(entry->names.add(entry.getKey().getName()));
        return names;
    }
